...
*   **PDF 解析**: Result Book (PDF形式) を読み込み、大会情報、試合結果、スコア、ショット情報を自動抽出します。
...
> [!WARNING]
> **データベースファイルの分離について**  
> 4人制と混合ダブルス（MD）では、エンド情報のデータ構造（`is_power_play` カラムの有無）が異なります。**4人制のデータとMDのデータは、必ず異なるデータベースファイル (`.db`) に保存してください。**
> 既存のデータベースに異なる形式のデータを追加しようとすると、エラーが発生したりデータが正しく保存されない可能性があります。
